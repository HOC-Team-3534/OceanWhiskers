...
public class CharacterizeDrive extends Command {
    ...
    public CharacterizeDrive(
            Swerve swerveDrive, Per<VoltageUnit, TimeUnit> quas_voltage, Time quas_duration) {
    ...
    @Override
    public void execute() {
        var requested_voltage =
                quas_voltage.in(Volts.per(Second)) * characterizer.getTimeSinceStart().in(Seconds);
