...
public class Forbar extends TalonFXMechanism {
        public static class ForbarConfig extends TalonFXMechanism.Config {
                @Getter
                private Voltage outVoltage = Volts.of(6.0); // out is positive
                @Getter
                private Voltage holdOutVoltage = Volts.of(2.0); // out is positive
                @Getter
                private Voltage inVoltage = Volts.of(-3.0); // in is negative
                @Getter
                private Power spikeThreshold = outVoltage.times(Amps.of(10.0));

                Pose3d fieldToRobotCAD = new Pose3d(0.0, 0.0, 0.0414, new Rotation3d());

                Pose3d robotToLongPivot = fieldToRobotCAD.transformBy(
                                GeomUtil.toTransform3d(new Translation3d(0.17, 0.0, 0.4425)));
                Pose3d robotToShortPivot = fieldToRobotCAD.transformBy(
                                GeomUtil.toTransform3d(new Translation3d(0.28375, 0.0, 0.4325)));
                Pose3d robotToCarriage = fieldToRobotCAD.transformBy(
                                GeomUtil.toTransform3d(new Translation3d(0.22, 0.0, 0.6525)));

                Translation3d carriageToBottomOfCoral = new Translation3d(Inches.of(1.625), Inches.zero(),
                                Inches.of(-2.647));
        ...
        public void simulationPeriodic() {
                Robot.getDtm()
                                ...
                                .ifPresentOrElse(
                                                loadingStationPose -> {
                                                        var relative = loadingStationPose
                                                                        .relativeTo(Robot.getSwerve().getPose());

                                                        if (relative.getMeasureX().isNear(Inches.zero(), Inches.of(2.0))
                                                                        && relative.getMeasureY().isNear(Inches.zero(),
                                                                                        Inches.of(24.0))
        ...
        public class State {
                ...
                CachedValue<ForbarComponentOffsets> componentOffsets = new CachedValue<>(
                                () -> updateForbarComponentPositions(getPercentOutVsIn()));
                CachedValue<Optional<Entry<ReefLevel, Pose3d>>> validScoringLocation = new CachedValue<>(
                                () -> updateValidScoringLocation());
                ...
                private double getPercentOutVsIn() {
                        return switch (position) {
                                case GoingIn:
                                        yield 1
                                                        - Math.min(
                                                                        currentPositionTimer.get()
                                                                                        / config.timeFromInToOut
                                                                                                        .in(Seconds),
                                                                        1);
                                case GoingOut:
                                        yield Math.min(
                ...
                Optional<Entry<ReefLevel, Pose3d>> updateValidScoringLocation() {
                        return DTM.getClosestReefBranch()
                                        .flatMap(
                                                        branch -> {
                                                                var bottomOfCoral = new Pose3d(
                                                                                Robot.getSwerve().getPose())
                                                                                .transformBy(
                                                                                                GeomUtil.toTransform3d(
                                                                                                                getComponentOffsets().bottomOfCoral));
                ...
                ForbarComponentOffsets updateForbarComponentPositions(double percentOutVsIn) {
                        Angle changeInShortPivotPitch = config.shortPivotPitchRange
                                        .times(MathUtil.clamp(percentOutVsIn, 0.0, 1.0));

                        var shortPivotOffset = config.robotToShortPivot.transformBy(
                                        GeomUtil.toTransform3d(0.0, changeInShortPivotPitch, 0.0));

                        var endOfShortPivot = shortPivotOffset
                                        .transformBy(
                        ...
                        var endOfLongPivot = GeomUtil.calcIntercetionOfCirclesInXZPlane(
                        ...
                        var longPivotOffset = new Pose3d(
                                        config.robotToLongPivot.getTranslation(),
                                        new Rotation3d(
                                                        0.0,
                        ...
                        var toEndOfLongPivotFromEndofShortPivot = endOfLongPivot
                                        .minus(endOfShortPivot.getTranslation());

                        var carriageOffset = new Pose3d(
                                        endOfShortPivot.getTranslation(),
                                        new Rotation3d(
                                                        0.0,
                                                        Units.degreesToRadians(40)
                                                                        + Math.atan2(
                                                                                        toEndOfLongPivotFromEndofShortPivot
                                                                                                        .getX(),
                                                                                        toEndOfLongPivotFromEndofShortPivot
                                                                                                        .getZ()),
                                                        0.0));

                        var bottomOfCoral = carriageOffset
