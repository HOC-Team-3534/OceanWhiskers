 ...
 * Electronics devices. We recommend using the Swerve Project Generator in Tuner X to create a
 * template project that demonstrates how to use this class.
 *
 ...
 *
 * <p>This class performs pose estimation internally using a separate odometry thread. Vision
 * measurements can be added using {@link #addVisionMeasurement}. Other odometry APIs such as {@link
 * #resetPose} are also available. The resulting pose estimate can be retrieved along with module
 * states and other information using {@link #getState}. Additionally, the odometry thread
 * synchronously provides all new state updates to a telemetry function registered with {@link
 * #registerTelemetry}.
 *
 ...
 * get the third (0-indexed) module.
 */
public abstract class SwerveDrivetrain<
        ...
        extends HocSubsystem implements AutoCloseable {
    ...
    public static class SwerveDriveState implements Cloneable {
        ...
        /**
         * The timestamp of the state capture, in the timebase of {@link
        ...
        /** Creates a deep copy of this state object. This API is not thread-safe. */
        @Override
        public SwerveDriveState clone() {
            final var toReturn = new SwerveDriveState();
            toReturn.Pose = Pose;
        ...
        protected void updateFromJni(SwerveJNI.DriveState driveState) {
            if (Pose.getX() != driveState.PoseX
                ...
                Pose =
                        new Pose2d(
                                driveState.PoseX,
                                driveState.PoseY,
                                Rotation2d.fromRadians(driveState.PoseTheta));
            }
            ...
            if (Speeds.vxMetersPerSecond != driveState.SpeedsVx
                ...
                Speeds =
                        new ChassisSpeeds(
                                driveState.SpeedsVx, driveState.SpeedsVy, driveState.SpeedsOmega);
            }
                ...
                if (ModuleStates[i].speedMetersPerSecond != driveState.ModuleStates[i].speed
                    ...
                    ModuleStates[i] =
                            new SwerveModuleState(
                                    driveState.ModuleStates[i].speed,
                                    Rotation2d.fromRadians(driveState.ModuleStates[i].angle));
                }
            }
            for (int i = 0; i < ModuleTargets.length; ++i) {
                if (ModuleTargets[i].speedMetersPerSecond != driveState.ModuleTargets[i].speed
                        || ModuleTargets[i].angle.getRadians()
                                != driveState.ModuleTargets[i].angle) {
                    ModuleTargets[i] =
                            new SwerveModuleState(
                                    driveState.ModuleTargets[i].speed,
                                    Rotation2d.fromRadians(driveState.ModuleTargets[i].angle));
                }
            }
            for (int i = 0; i < ModulePositions.length; ++i) {
                if (ModulePositions[i].distanceMeters != driveState.ModulePositions[i].distance
                        || ModulePositions[i].angle.getRadians()
                                != driveState.ModulePositions[i].angle) {
                    ModulePositions[i] =
                            new SwerveModulePosition(
                                    driveState.ModulePositions[i].distance,
                                    Rotation2d.fromRadians(driveState.ModulePositions[i].angle));
                }
            }
        ...
        protected void updateFromJni(SwerveJNI.ControlParams controlParams) {
            ...
            if (operatorForwardDirection.getRadians() != controlParams.operatorForwardDirection) {
                operatorForwardDirection =
                        Rotation2d.fromRadians(controlParams.operatorForwardDirection);
            }
    ...
    private final SwerveControlParameters m_controlParams = new SwerveControlParameters();
    ...
    private final SwerveDriveState m_cachedState = new SwerveDriveState();
    ...
    public class OdometryThread {
        ...
        /**
         * Sets the odometry thread priority to a real time priority under the specified priority
         * level
         *
         * @param priority Priority level to set the odometry thread to. This is a value between 0
         *     and 99, with 99 indicating higher priority and 0 indicating lower priority.
         */
    ...
    private final OdometryThread m_odometryThread = new OdometryThread();
        ...
        this(
                ...
                () -> {
                    ...
                    long nativeModuleConstants =
                            SwerveModuleConstants.createNativeInstance(modules);

                    var drivetrain =
                            SwerveJNI.JNI_CreateDrivetrain(
                                    nativeDriveConstants, nativeModuleConstants, modules.length);
    ...
    public SwerveDrivetrain(
        ...
        this(
                ...
                () -> {
                    ...
                    var drivetrain =
                            SwerveJNI.JNI_CreateDrivetrainWithFreq(
                                    nativeDriveConstants,
    ...
    public SwerveDrivetrain(
        ...
        this(
                ...
                () -> {
                    ...
                    var drivetrain =
                            SwerveJNI.JNI_CreateDrivetrainWithStddev(
                                    nativeDriveConstants,
                                    odometryUpdateFrequency,
                                    odometryStandardDeviation.getData(),
                                    visionStandardDeviation.getData(),
                                    nativeModuleConstants,
                                    modules.length);
    ...
    private SwerveDrivetrain(
        ...
        var tmpModules =
                (SwerveModule<DriveMotorT, SteerMotorT, EncoderT>[])
                        new SwerveModule<?, ?, ?>[modules.length];

        m_modules = tmpModules;
        m_moduleLocations = new Translation2d[modules.length];
        for (int i = 0; i < modules.length; ++i) {
            m_modules[i] =
                    new SwerveModule<DriveMotorT, SteerMotorT, EncoderT>(
                            driveMotorConstructor,
                            steerMotorConstructor,
                            encoderConstructor,
                            modules[i],
     ...
     */
    public final OdometryThread getOdometryThread() {
    ...
    /**
     * Gets a copy of the current state of the swerve drivetrain. This includes information such as
     * the pose estimate, module states, and chassis speeds.
     *
     * <p>This can be used to get a thread-safe copy of the state object.
     *
     * @return Copy of the current state of the drivetrain
     */
    public final SwerveDriveState getStateCopy() {
        m_jni.JNI_GetState(m_drivetrainId);
        try {
            m_stateLock.lock();
            m_cachedState.updateFromJni(m_jni.driveState);
    ...
    public StatusCode configNeutralMode(NeutralModeValue neutralMode) {
        return StatusCode.valueOf(
                SwerveJNI.JNI_ConfigNeutralMode(m_drivetrainId, neutralMode.value));
    }
     ...
     * SwerveRequest.ForwardPerspectiveValue#OperatorPerspective} perspective. This makes the
     * current orientation of the robot X forward for field-centric maneuvers.
     *
     * <p>This is equivalent to calling {@link #resetRotation} with the operator perspective
     * rotation.
     */
    ...
    public void resetPose(Pose2d pose) {
        SwerveJNI.JNI_ResetPose(
                m_drivetrainId, pose.getX(), pose.getY(), pose.getRotation().getRadians());
    }

    /**
     * Resets the translation of the robot pose without affecting rotation. The translation should
     * be from the {@link SwerveRequest.ForwardPerspectiveValue#BlueAlliance} perspective.
     *
     ...
     * is in the Red Alliance Station, this should be 180 degrees.
     *
     * @return Heading indicating which direction is forward from the {@link
     *     SwerveRequest.ForwardPerspectiveValue#BlueAlliance} perspective
     */
    public final Rotation2d getOperatorForwardDirection() {
        double operatorForwardDirectionRad =
                SwerveJNI.JNI_GetOperatorForwardDirection(m_drivetrainId);
        try {
            m_stateLock.lock();
     ...
     * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that you
     *     must use a timestamp with an epoch since system startup (i.e., the epoch of this
     *     timestamp is the same epoch as {@link Utils#getCurrentTimeSeconds}). This means that you
     *     should use {@link Utils#getCurrentTimeSeconds} as your time source or sync the epochs. An
     *     FPGA timestamp can be converted to the correct timebase using {@link
     *     Utils#fpgaToCurrentTime}.
     */
    public void addVisionMeasurement(Pose2d visionRobotPoseMeters, double timestampSeconds) {
        SwerveJNI.JNI_AddVisionMeasurement(
                m_drivetrainId,
    ...
    public void addVisionMeasurement(
        ...
        SwerveJNI.JNI_AddVisionMeasurementWithStdDev(
                m_drivetrainId,
                visionRobotPoseMeters.getX(),
                visionRobotPoseMeters.getY(),
                visionRobotPoseMeters.getRotation().getRadians(),
    ...
    public void setVisionMeasurementStdDevs(Matrix<N3, N1> visionMeasurementStdDevs) {
        SwerveJNI.JNI_SetVisionMeasurementStdDevs(
                m_drivetrainId, visionMeasurementStdDevs.getData());
    }

    /**
     * Return the pose at a given timestamp, if the buffer is not empty.
     *
     * @param timestampSeconds The pose's timestamp. Note that you must use a timestamp with an
     *     epoch since system startup (i.e., the epoch of this timestamp is the same epoch as {@link
    ...
    /**
     * Get a reference to the full array of modules. The indexes correspond to the module described
     * in the constructor.
     *
    ...
    /**
     * Gets the current orientation of the robot as a {@link Rotation3d} from the Pigeon 2
     * quaternion values.
     *
